...
class bldg_grid_agg_data_driven_mdl_large:
    def __init__(
        self,
        ms_dot,
        T_sa,
        ...
        self.truth_model_Pwr = np.array(20.0)
    ...
    def reinit(self, inputs, disturbances):
        ...
        self.Bu = np.array([[0.00002533, 0.0, 0.0]])  # 1 minute model
        ...
        self.Bd_mean_inputs = np.array([[26.3525], [0.0], [0.0], [0.0]])
        ...
        self.Cy = np.array([[1.0], self.Cz[0], -1 * self.Cz[0], self.Cz[0]])
        self.Dyu = np.array(
            [[0.0, 0.0, 0.0], self.Dzu[0], -1 * self.Dzu[0], self.Dzu[0]]
        )
        self.Dyv = np.array(
            [[0.0], self.Dzv[0], [1.0 - self.Dzv[0]], self.Dzv[0]]
        )
        self.Dyd = np.array(
            [
                [0.0, 0.0, 0.0, 0.0],
        ...
        self.Cy_lin = dot(
            np.array([[0.0], [self.Cy[1]], [-1 * self.Cy[1]], [self.Cy[1]]]),
            self.T_z_lin,
        )
        ...
        self.Dyu_lin = dot(
            np.array(
                [
                    np.zeros(np.shape(self.Dyu)[1]).tolist(),
                    self.Dyu[1],
                    -1 * self.Dyu[1],
                    self.Dyu[1],
                ]
            ),
            np.array([[0.0], [self.ms_dot_lin], [self.T_sa_lin]]),
        )
        # print(np.shape(self.Dyu_lin))
        # print(self.Dyu_lin)
        # lkj
        self.Dzu_lin = dot(
            self.Dzu, np.array([[0.0], [self.ms_dot_lin], [self.T_sa_lin]])
        )
        ...
        self.Dyd_lin = dot(
            self.Dyd,
            np.array([[self.T_oa_lin], [0.0], [0.0], [self.emissions_lin]]),
        )
        self.Dzd_lin = dot(
            self.Dzd, np.array([[self.T_oa_lin], [0.0], [0.0], [0.0]])
        )
    ...
    def filter_update(self, states, outputs):
        T_z_truth = outputs[0]
        states = states + self.K * (
            T_z_truth - self.Cy_mean_outputs[0][0] - dot(self.Cy[0], states)
        )
        return states
        ...
        refs = (
            ...
            - np.array(
                [
                    0,
                    self.truth_model_Pwr,
                    -1 * self.truth_model_Pwr,
                    self.truth_model_Pwr,
                ]
                * self.horiz_len
            ).reshape(np.shape(refs_const)[0], 1)
            + np.array([self.Cy_lin] * self.horiz_len).reshape(
                np.shape(refs_const)[0], 1
            )
            + np.array([self.Dyu_lin] * self.horiz_len).reshape(
                np.shape(refs_const)[0], 1
            )
            + np.array([self.Dyd_lin] * self.horiz_len).reshape(
                np.shape(refs_const)[0], 1
            )
        )
        return refs
    ...
    def get_forecast(self, current_time, disturbance_data):
        ...
        tmp = disturbance_data.iloc[
            current_time : current_time + self.horiz_len
        ]
        vars = []
        for key in self.disturb_keys:
            if key is "Q_internal":
                vars.append(
                    tmp[key].values * self.Qint_scale + self.Qint_offset
                )
            elif key is "Q_solar":
                vars.append(
                    tmp[key].values * self.Qsol_scale + self.Qsol_offset
                )
            else:
                offset = 0.0
                vars.append(tmp[key].values)

        return np.array([val for tup in zip(*vars) for val in tup]).reshape(
            np.shape(vars)[0] * np.shape(vars)[1], 1
        )
    ...
    def compute_cons(self, funcs, uOpt, Y):
        ...
        funcs["hvac_con"] = (
            np.array(Qhvac)
            - np.array(ms_dot) * (np.array(T_sa) - (self.truth_model_T_z))
        ).reshape(self.horiz_len)
    ...
    def bldg_chiller_emissions(self, inputs):
        ...
        emissions = inputs[4]
        T_ma = 0.3 * T_oa + (1 - 0.3) * T_z
        emissions_chill = emissions * (
            1.005 / self.hvac_cop * ms_dot * (T_ma - T_sa)
        )
    ...
    def bldg_power_model(self, ms_dot, T_sa, T_oa, T_z, emissions):
        ...
        D_fan_emissions = grad_fan_emissions(
            [T_sa, ms_dot, T_oa, T_z, emissions]
        )
        D_chill_emissions = grad_chill_emissions(
            [T_sa, ms_dot, T_oa, T_z, emissions]
        )

        Dz = np.array(D_fan_power) + np.array(D_chill_power)
        self.Dz_lin_grads = np.append(
            Dz, np.array(D_fan_emissions)[-1] + np.array(D_chill_emissions)[-1]
        )
    ...
    def process_uConv(self, uConv):
        uConv[0::3] = np.array(uConv[1::3]) * (
            np.array(uConv[2::3]) - (self.truth_model_T_z)
        )
